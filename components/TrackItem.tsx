...
import { Plus, Minus, Play, Clock, Disc, BarChart2, ExternalLink } from 'lucide-react';
...
export const TrackItem: React.FC<TrackItemProps> = ({ track, isOpen, onToggle, index }) => {
  return (
    <motion.div 
    ...
    >
      ...
      <div className={`absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-primary to-accent transition-all duration-300 ${isOpen ? 'opacity-100 shadow-[0_0_15px_rgba(45,212,191,0.8)]' : 'opacity-0 group-hover:opacity-100'}`} />

      {/* Main Row */}
      <motion.div 
        onClick={onToggle}
        whileHover={{ backgroundColor: isOpen ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.02)' }}
      ...
      >
        ...
        <div className="col-span-8 md:col-span-5 flex flex-col md:flex-row md:items-baseline gap-2">
          <h3 className={`text-xl md:text-3xl font-bold uppercase tracking-tight transition-all duration-300 ${isOpen ? 'text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent neon-text' : 'text-white group-hover:text-white'}`}>
            {track.title}
          </h3>
          <span className="hidden md:inline-block text-muted-foreground/40 mx-2">—</span>
          ...
          >
            ...
            <div className="px-4 md:px-8 pb-12 pt-4 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 relative z-10">
                ...
                <div className="col-span-12 md:col-span-4 relative group/image">
                  ...
                  <div className="mt-6 grid grid-cols-2 gap-4 border-t border-white/10 pt-4">
                    <div className="flex items-center gap-2 text-xs font-mono text-primary/80">
                       <Clock size={12} className="text-primary" /> {track.duration}
                    </div>
                    <div className="flex items-center gap-2 text-xs font-mono text-primary/80">
                       <BarChart2 size={12} className="text-primary" /> {track.bpm} BPM
                    </div>
                ...
                {/* Details Area */}
                <div className="col-span-12 md:col-span-5 bg-white/[0.03] p-6 md:p-10 border border-white/5 hover:border-white/10 transition-colors">
                  <h4 className="text-xs font-mono text-primary uppercase mb-6 tracking-widest neon-text">Synopsis</h4>
                  <p className="text-lg md:text-xl font-light leading-relaxed text-gray-200 mb-8 selection:bg-primary/30">
                    {track.description}
                  </p>

                  <h4 className="text-xs font-mono text-primary uppercase mb-4 tracking-widest neon-text">Credits</h4>
                  <div className="flex flex-wrap gap-2">
                    {track.credits.map((credit, i) => (
                      <span key={i} className="px-3 py-1 border border-white/10 rounded-full text-xs text-muted-foreground hover:text-primary hover:border-primary/50 transition-colors cursor-default hover:shadow-[0_0_10px_rgba(45,212,191,0.1)]">
                        {credit}
                      </span>
                    ))}
                  </div>
                ...
                <div className="col-span-12 md:col-span-3 flex flex-col justify-between h-full min-h-[200px]">
                   <div className="space-y-4">
                      <h4 className="text-xs font-mono text-primary uppercase tracking-widest neon-text">Listen</h4>
                      <div className="flex flex-col gap-3">
                          {['Spotify', 'Apple Music', 'Soundcloud'].map((platform) => (
                            ...
                            >
                                {platform} <ExternalLink size={14} className="text-primary opacity-0 group-hover/btn:opacity-100 transition-opacity" />
                            </motion.button>
                          ))}
                      </div>
                   </div>
                   
                   <div className="mt-8 md:mt-0 p-4 border border-dashed border-white/10 bg-black/40">
                      <div className="text-[10px] uppercase text-muted-foreground mb-2">Curated On</div>
                      <div className="font-mono text-sm text-primary neon-text">24 OCT 2025</div>
                   </div>
                </div>
