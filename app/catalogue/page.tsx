...
const GENRES = ['IDM', 'Ambient', 'Techno', 'Deep House', 'Future Garage', 'Synthwave', 'Experimental', 'Drone'];
const ARTISTS = ['Burial', 'Aphex Twin', 'Boards of Canada', 'Four Tet', 'Jon Hopkins', 'Floating Points', 'Caribou', 'Moderat', 'Bicep', 'Jamie xx', 'Nicolas Jaar', 'Bonobo', 'Tycho', 'Rival Consoles'];
const ADJECTIVES = ['Ethereal', 'Dusty', 'Analog', 'Sublime', 'Glitchy', 'Hypnotic', 'Kinetic', 'Nocturnal', 'Hazed', 'Liquid'];
const NOUNS = ['Echo', 'Signal', 'Memory', 'Drift', 'Pulse', 'Loop', 'Artifact', 'Phase', 'Transmission', 'State'];
...
const generateTracks = (count: number, startId: number): Track[] => {
  return Array.from({ length: count }).map((_, i) => {
    ...
    return {
      ...
      duration: `${3 + Math.floor(Math.random() * 4)}:${10 + Math.floor(Math.random() * 50)}`,
      genre,
      bpm: 90 + Math.floor(Math.random() * 50),
      description: `A procedurally recovered entry from the permanent record. This track exemplifies the ${genre.toLowerCase()} aesthetic with ${ADJECTIVES[Math.floor(Math.random() * ADJECTIVES.length)].toLowerCase()} textures and high-fidelity transients.`,
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10">
            ...
            <div className="mb-16">
                ...
                >
                    <div>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="text-xs font-semibold font-mono uppercase text-primary tracking-[0.3em]">Full Archive</span>
                        </div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                         <div className="text-right">
                             <span className="text-xs font-mono text-muted-foreground uppercase tracking-widest block mb-1">Total Records</span>
                             <span className="text-3xl font-bold text-white neon-text">{tracks.length}+</span>
                         </div>
                    </div>
                </motion.div>
                ...
                <motion.div 
                    ...
                    className="flex flex-col md:flex-row gap-4 items-center bg-white/[0.02] p-4 border border-white/10"
                >
                    ...
                    <motion.button 
                        whileHover={{ scale: 1.02, backgroundColor: "rgba(255, 255, 255, 0.05)", borderColor: "rgba(45, 212, 191, 0.3)" }}
                        whileTap={{ scale: 0.98 }}
